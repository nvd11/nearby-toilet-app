...
def process_data(data):
    ...
    # Boss logic check: REGMAP MI must have 'regmap' as upstream. 
    # If not present, add it. The inbound pattern will be assumed DB (which maps to 37.0 MD).
    if 'REGMAP MI' in consumer_map:
        # Check if 'regmap' (case-insensitive) is in sources
        found = any(src.lower() == 'regmap' for src in consumer_map['REGMAP MI']['sources'].keys())
        if not found:
            consumer_map['REGMAP MI']['sources']['REGMAP'] = 37.0 # Assumed default cost for DB/File
...
def get_consumer_cost(consumer, sources_dict, outbound_md, seen_sources):
    ...
    for src, base_md in sources_dict.items():
        if consumer.lower() == 'regmap mi' and src.lower() == 'helios':
            ...
            inbound_total += 20.0 # Similar to RAPID2 MI -> RAPID2 logic, assuming existing connection or heavy data migration
...
def generate_v3_plan(consumer_map, out_file):
    ...
    while unprocessed:
        ...
        for consumer in unprocessed:
            ...
            cost = get_consumer_cost(consumer, sources_dict, outbound_md, seen_sources)
        ...
        for src in sources_dict.keys():
            if src not in seen_sources:
                seen_sources[src] = best_consumer
        ...
        for consumer in ordered_consumers:
            ...
            for src in sources_list:
                ...
                if consumer.lower() == 'regmap mi' and src.lower() == 'helios':
                    ...
                    seen_sources_for_writing[src] = consumer
                    row_remarks[src] = "RCDP already has REGMAP connection; 20 MD for heavy table creation & history data migration (Added per Jason's correction)."
                elif consumer.lower() == 'regmap' and src.lower() == 'rdh and helios(rdm)':
                    md = 10.0
                    inbound_parts.append("10")
                    seen_sources_for_writing[src] = consumer
