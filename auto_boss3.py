...
with sync_playwright() as p:
    ...
    try:
        page.goto("https://www.zhipin.com/web/geek/resume", wait_until="domcontentloaded", timeout=15000)
    except Exception as e:
        print("Goto timeout, but continuing:", e)
        
    page.wait_for_timeout(5000)
    print("Current URL:", page.url)
    
    # Try finding Edit button
    try:
        print("Looking for Edit button...")
        # Check if the text "个人优势" exists
        if page.locator("text=个人优势").count() > 0:
            print("Found '个人优势' text.")
            
            # Using evaluate to find and click the edit button
            js_code = """() => {
                let els = Array.from(document.querySelectorAll('*')).filter(el => el.innerText === '个人优势');
            ...
            if page.locator("textarea").is_visible():
                print("Filling text...")
                page.locator("textarea").fill(text_to_fill)
                page.wait_for_timeout(1000)
                
                print("Clicking Save...")
                # The save button usually says '保存' or '完成'
            ...
            print("Could not find '个人优势' on the page.")
    except Exception as e:
        print("Error during automation:", e)
        
    page.screenshot(path="/home/gateman/.openclaw/workspace/boss_resume_debug2.png")
