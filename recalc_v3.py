...
def calculate_effort(inbound, outbound):
    # Baselines based on the new image (IMG_20260429_011413.jpg)
    # Upstream: File=34, PubSub=29.5, API=30
    # Downstream: Auth Views=17.5, File Export=19.5, API Provision=20.5
    
    in_md = 34 # default to file if unknown
    in_lower = inbound.lower()
    if 'file' in in_lower:
        in_md = 34
    elif 'db' in in_lower or 'table' in in_lower:
        # DB is not allowed, but if it exists in legacy mapping, we must convert it to an allowed pattern.
        # Let's assume we force them to use File export (NAS/SFTP) as the fallback.
        in_md = 34 
