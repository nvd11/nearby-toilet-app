...
with sync_playwright() as p:
    ...
    print("Found page:", target_page.url)
    ...
    for i in range(max_wait):
        if "login" not in target_page.url and "404" not in target_page.url:
            # Check if headers exist
            headers = target_page.locator(".resume-item-title, .title, h3").count()
            if headers > 0:
                logged_in = True
                break
        time.sleep(1)
    ...
    print("Logged in! Executing JS to click Edit...")
    js_click_edit = """() => {
        let headers = Array.from(document.querySelectorAll('.resume-item-title, .title, h3')).filter(el => el.innerText && el.innerText.includes('个人优势'));
        ...
        let editBtn = container.querySelector('.op-edit, .icon-edit, i.icon-edit, .btn-edit');
        if (!editBtn) {
    ...
    js_fill_save = """(text) => {
        let textarea = document.querySelector('textarea.input-area, textarea.textarea, textarea, [contenteditable="true"]');
        if (!textarea) return 'Textarea not found';
        
        let nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, "value");
        if (nativeInputValueSetter && nativeInputValueSetter.set) {
            nativeInputValueSetter.set.call(textarea, text);
        } else {
            textarea.value = text;
        }
        ...
        let saveBtns = Array.from(formContainer.querySelectorAll('button, a, span.btn')).filter(el => el.innerText && (el.innerText.includes('保存') || el.innerText.includes('完成')));
        
        if (saveBtns.length > 0) {
            let saveBtn = saveBtns.find(b => b.className.includes('primary') || b.className.includes('submit')) || saveBtns[saveBtns.length - 1];
            saveBtn.click();
    ...
    time.sleep(2)
    target_page.screenshot(path="/home/gateman/.openclaw/workspace/boss_stealth_done3.png")
    print("Done!")
