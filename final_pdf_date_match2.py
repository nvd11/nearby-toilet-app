...
out_path = '/home/gateman/.openclaw/workspace/signed_document_v10.pdf'

doc = fitz.open(pdf_path)
page = doc[-1]

font_path = "/home/gateman/.openclaw/workspace/Caveat-Regular.ttf"

# The signature is grey and thin. Let's use a lighter grey.
# RGB around 0.45, 0.45, 0.45
col = (0.45, 0.45, 0.45)

box_rects = page.search_for("☐")
for br in box_rects:
    bp = fitz.Point(br.x0 + 2, br.y1 - 3)
    page.insert_text(bp, "√", fontfile=font_path, fontname="hw_c", fontsize=15, color=col)

date_rects = page.search_for("日期：")
if date_rects:
    dr = date_rects[-1]
    dp = fitz.Point(dr.x1 + 10, dr.y1 - 2)
    # Using a slightly smaller font size to keep it thin
    page.insert_text(dp, "2026.04.23", fontfile=font_path, fontname="hw_c", fontsize=16, color=col)
