...
with sync_playwright() as p:
    ...
    if not target_page:
        print("Could not find any Zhipin page. Creating a new one.")
        target_page = context.new_page()
        target_page.goto("https://www.zhipin.com/web/geek/resume", timeout=60000)
    ...
    # If not on resume page, try clicking it
    if "resume" not in target_page.url:
        print("Clicking resume tab...")
        try:
            target_page.locator("text=简历").first.click(timeout=3000)
            target_page.wait_for_timeout(3000)
        except:
            pass
    ...
    target_page.wait_for_timeout(3000)
    target_page.screenshot(path="/home/gateman/.openclaw/workspace/boss_final_result3.png")
    print("Done!")
