...
def calculate_effort(inbound, outbound):
    # Baselines
    # Upstream: File=28, DB=19, API=20 (assumed)
    # Downstream: Table/View=7, File=11
...
def process_data(data):
    # map: consumer -> { 'sources': [], 'total_md': 0, 'outbound_md': 0, 'inbound_mds': [] }
    ...
    for row in data:
        ...
        for consumer in consumers:
            ...
            if c:
                ...
                consumer_map[c]['outbound_md'] = out_md # assume same for the consumer
...
def generate_v3_plan(consumer_map, out_file):
    ...
    with open(out_file, 'w', encoding='utf-8') as f:
        ...
        f.write("Calculations are based on the following finalized baselines:\n")
        f.write("*   **Upstream File (NAS/SFTP):** 28.0 MD\n")
        f.write("*   **Upstream Database (JDBC):** 19.0 MD\n")
        f.write("*   **Upstream API (Assumed):** 20.0 MD\n")
        f.write("*   **Downstream Auth Views:** 7.0 MD\n")
        f.write("*   **Downstream File Export:** 11.0 MD\n\n")
        ...
        for consumer in sorted_consumers:
            sources_str = ", ".join(sorted(list(normalized_map[consumer]['sources'])))
            ...
            f.write(f"| {consumer} | {sources_str} | **{total_md} MD** | {batch} | {dv_date} | {demise_date} |\n")
