...
with sync_playwright() as p:
    ...
    js_fill_save = """(text) => {
        ...
        // React/Vue often overrides value setter, so we need to bypass it
        let nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLTextAreaElement.prototype, "value").set;
        nativeInputValueSetter.call(textarea, text);
        
        // Dispatch events so the frontend framework picks up the change
        textarea.dispatchEvent(new Event('input', { bubbles: true }));
        textarea.dispatchEvent(new Event('change', { bubbles: true }));
        
        // Find save button (usually nearby or at the bottom of the form)
        ...
        if (saveBtns.length > 0) {
            // Usually the primary button or the last one matching
    ...
    target_page.screenshot(path="/home/gateman/.openclaw/workspace/boss_final_result.png")
    print("Done!")
