...
with sync_playwright() as p:
    ...
    # Boss shared a screenshot, he has the window open but maybe the URL changed slightly.
    # Let's execute JS on all pages to find the one with "个人优势"
    target_page = None
    for page in context.pages:
        try:
            has_advantage = page.evaluate("() => document.body.innerText.includes('个人优势')")
            if has_advantage:
                target_page = page
                break
        except Exception:
    ...
    if not target_page:
        print("Could not find any page with '个人优势' visible. Exiting.")
        exit(1)
    ...
    target_page.screenshot(path="/home/gateman/.openclaw/workspace/boss_final_result_force.png")
    print("Done!")
